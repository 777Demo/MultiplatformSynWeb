...
//type TSynWebBase1 = class(TSynWebBase);
...
begin
  ...
  begin
    if APos.Line = 1 then
      ...
      //SetRange(TSynEditStringList(Lines).Range[CaretY-2]);
    //  TSynWebBase1(ASynWeb).CurrentRanges[1];
...
var t:TBufferCoord;
begin
  t.Char:=ASynEdit.CaretX;
  t.Line:=ASynEdit.CaretY;
  Result := SynWebGetHighlighterTypeAt(ASynEdit, ASynWeb, t);
end;
...
function SynWebGetUnmatchedTags(ASynEdit: TCustomSynEdit;
  ...
  var BufferCoord:TBufferCoord;
begin
  BufferCoord.Char :=  ASynEdit.CaretX;
  BufferCoord.Line :=  ASynEdit.CaretY;
...
begin
  lT := TSynWebTokenizer.Create(AEditor, TBufferCoord(AEditor.CaretXY));
  lT.MarkerPos := TBufferCoord(AEditor.CaretXY);
